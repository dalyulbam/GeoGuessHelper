---
{
  "body": "Poland uses right-hand driving, white lane markings, circular red-bordered speed limit signs, and EU-standard blue rectangular pedestrian/cyclist signage, plus white rectangular EU-format license plates — consistent with broader EU/Central European road signage conventions rather than distinguishing Poland specifically from other EU member states.",
  "category": "road-signage",
  "cell": "u3jy6rm",
  "created": 1786642690.2837303,
  "entities": [
    "poland"
  ],
  "heading": null,
  "id": "atm_e29dd72ae7f9",
  "lang": "en",
  "lat": 51.7642643,
  "layer": "architecture",
  "lng": 19.4423915,
  "period_end": null,
  "period_start": null,
  "radius_km": 1200,
  "refs": [],
  "reports": [
    "report_pl_lodz_51.7643_19.4424_260814_023750_ko.html",
    "report_pl_mysliborz_52.9267_14.8708_260814_233100_ko.html"
  ],
  "scope": "country",
  "sources": [],
  "tags": [
    "driving-side",
    "eu-standard",
    "poland",
    "road-signage"
  ],
  "title": "Polish EU-standard road signage and driving conventions",
  "updated": 1788506030.223211,
  "uses": 2
}
---

# Polish EU-standard road signage and driving conventions

`atm_e29dd72ae7f9` · **architecture** / country · cell `u3jy6rm`

Poland uses right-hand driving, white lane markings, circular red-bordered speed limit signs, and EU-standard blue rectangular pedestrian/cyclist signage, plus white rectangular EU-format license plates — consistent with broader EU/Central European road signage conventions rather than distinguishing Poland specifically from other EU member states.

**태그**: #driving-side #eu-standard #poland #road-signage

## 이 원자를 쓴 보고서
- `report_pl_lodz_51.7643_19.4424_260814_023750_ko.html`
- `report_pl_mysliborz_52.9267_14.8708_260814_233100_ko.html`
