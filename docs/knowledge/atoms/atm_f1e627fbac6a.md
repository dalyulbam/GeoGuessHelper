---
{
  "body": "Marathi is the official language of the Indian state of Maharashtra, written in the Devanagari script. Public signage typically appears in Marathi/Hindi (Devanagari) alongside English (Latin script), reflecting India's multilingual administrative norm. Common signage words include शासकीय (government) and संग्रहालय (museum).",
  "category": "script",
  "cell": "tet94mc",
  "created": 1786293223.7447946,
  "entities": [
    "devanagari-script",
    "maharashtra",
    "marathi-language"
  ],
  "heading": null,
  "id": "atm_f1e627fbac6a",
  "lang": "en",
  "lat": 19.8953229,
  "layer": "language",
  "lng": 75.3355104,
  "period_end": null,
  "period_start": null,
  "radius_km": 400.0,
  "refs": [],
  "reports": [
    "report_in_bibi-ka-maqbara_19.8953_75.3355_260810_013326_ko-en-fr.html"
  ],
  "scope": "region",
  "sources": [],
  "tags": [
    "devanagari",
    "hindi",
    "india",
    "maharashtra",
    "marathi",
    "multilingual-signage"
  ],
  "title": "Marathi in Devanagari script in Maharashtra",
  "updated": 1788506030.9607673,
  "uses": 1
}
---

# Marathi in Devanagari script in Maharashtra

`atm_f1e627fbac6a` · **language** / region · cell `tet94mc`

Marathi is the official language of the Indian state of Maharashtra, written in the Devanagari script. Public signage typically appears in Marathi/Hindi (Devanagari) alongside English (Latin script), reflecting India's multilingual administrative norm. Common signage words include शासकीय (government) and संग्रहालय (museum).

**태그**: #devanagari #hindi #india #maharashtra #marathi #multilingual-signage

## 이 원자를 쓴 보고서
- `report_in_bibi-ka-maqbara_19.8953_75.3355_260810_013326_ko-en-fr.html`
